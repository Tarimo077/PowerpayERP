...
class Profile(TimeStamped):
    ...
    def clean(self):
        ...
        seen={self.pk}; current=self.manager if self.manager_id else None
        while current:
            if current.pk in seen: raise ValidationError("The reporting hierarchy cannot contain a cycle.")
            seen.add(current.pk); current=current.manager
...
class Timesheet(TenantModel):
    ...
    requested_approver = models.ForeignKey(Profile, null=True, blank=True, on_delete=models.SET_NULL, related_name="timesheets_to_approve")
    request_task = models.OneToOneField(Task, null=True, blank=True, on_delete=models.SET_NULL, related_name="requested_timesheet")
    ...
    initial_budget_days = models.DecimalField(max_digits=8, decimal_places=2, null=True, blank=True)
    expert_signature = models.ImageField(upload_to="timesheet_signatures/%Y/%m/", blank=True)
    manager_signature = models.ImageField(upload_to="timesheet_signatures/%Y/%m/", blank=True)
    consultant_name = models.CharField(max_length=180, blank=True)
    consultant_signature = models.ImageField(upload_to="timesheet_signatures/%Y/%m/", blank=True)

    class Meta: 
        ordering = ["-period_start"]
        constraints = [models.UniqueConstraint(fields=["organization","employee","period_start"],name="unique_monthly_timesheet_per_employee")]
...
class TimesheetEntry(TimeStamped):
    ...
    overnight_duty_station = models.BooleanField(default=False, verbose_name="Overnight stay in duty-station country")
    overnight_outside_duty_station = models.BooleanField(default=False, verbose_name="Overnight stay outside duty-station country")
    per_diem_requested = models.BooleanField(default=False, verbose_name="Per diem requested")
    location = models.CharField(max_length=180, blank=True, help_text="Country, city and region where services were provided")
    ...
    def clean(self):
        ...
        if self.days_worked <= 0 or self.days_worked > 1:
            raise ValidationError("Days worked must be greater than zero and no more than 1.0 per entry.")
        
        if self.timesheet_id and (self.date < self.timesheet.period_start or self.date > self.timesheet.period_end):
...
class LeaveAllocation(TenantModel):
    ...
    employee = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name="leave_allocations")
    ...
    assigned_by = models.ForeignKey(User, on_delete=models.PROTECT, related_name="assigned_leave_allocations")
    notes = models.TextField(blank=True)

    class Meta:
        ordering = ["-year", "employee__user__first_name", "leave_type"]
        constraints = [models.UniqueConstraint(fields=["organization", "employee", "leave_type", "year"], name="unique_employee_leave_allocation")]
...
class LeaveRequest(TenantModel):
    ...
    employee = models.ForeignKey(Profile, on_delete=models.PROTECT, related_name="leave_requests")
    requested_approver = models.ForeignKey(Profile, on_delete=models.PROTECT, related_name="leave_requests_to_review")
    leave_type = models.CharField(max_length=30, choices=LeaveAllocation.LEAVE_TYPES, default="annual")
    ...
    def count_working_days(start_date, end_date):
        if not start_date or not end_date or start_date > end_date:
            return 0
        return sum(1 for offset in range((end_date-start_date).days+1) if (start_date+timedelta(days=offset)).weekday() < 5)
    ...
    def clean(self):
        ...
        if self.requested_approver_id:
            ...
            if self.requested_approver.role not in ["manager", "admin"]:
                raise ValidationError("Leave requests must be assigned to a manager or administrator.")
        if self.start_date and self.end_date:
            if self.start_date > self.end_date:
                raise ValidationError("End date must be on or after the start date.")
            if self.start_date.year != self.end_date.year:
                raise ValidationError("A leave request cannot span two calendar years. Submit a separate request for each year.")
            calculated = self.count_working_days(self.start_date, self.end_date)
            if calculated < 1:
                raise ValidationError("The selected dates do not contain a working day.")
...
class PaymentVoucher(TenantModel):
    ...
    department = models.ForeignKey(Department,null=True,blank=True,on_delete=models.SET_NULL,related_name="payment_vouchers")
    requested_approver = models.ForeignKey(Profile,null=True,blank=True,on_delete=models.SET_NULL,related_name="payment_vouchers_to_approve")
    approval_task = models.OneToOneField(Task,null=True,blank=True,on_delete=models.SET_NULL,related_name="payment_voucher_approval")
    status = models.CharField(max_length=20,choices=STATUSES,default="draft")
    prepared_by = models.ForeignKey(User,on_delete=models.PROTECT,related_name="prepared_payment_vouchers")
    approved_by = models.ForeignKey(User,null=True,blank=True,on_delete=models.SET_NULL,related_name="approved_payment_vouchers")
    ...
    class Meta:
        ...
        constraints = [models.UniqueConstraint(fields=["organization","number"],name="unique_voucher_number_per_org")]

    def clean(self):
        if self.department_id and self.department.organization_id != self.organization_id:
            raise ValidationError("Department belongs to another organization.")
        if self.requested_approver_id:
            if self.requested_approver.organization_id != self.organization_id: raise ValidationError("Approver belongs to another organization.")
            if self.requested_approver.role not in ["manager","admin"]: raise ValidationError("The assigned approver must be a manager or organization administrator.")
...
class PaymentVoucherLine(TimeStamped):
    voucher = models.ForeignKey(PaymentVoucher,on_delete=models.CASCADE,related_name="lines")
...
class PaymentVoucherReceipt(TimeStamped):
    voucher = models.ForeignKey(PaymentVoucher,on_delete=models.CASCADE,related_name="receipts")
    file = models.FileField(upload_to="payment_voucher_receipts/%Y/%m/")
    original_name = models.CharField(max_length=255)
    uploaded_by = models.ForeignKey(User,on_delete=models.PROTECT,related_name="uploaded_voucher_receipts")
...
class ItemRequest(TenantModel):
    ...
    number=models.CharField(max_length=30)
    requested_by=models.ForeignKey(User,on_delete=models.PROTECT,related_name="item_requests")
    department=models.ForeignKey(Department,null=True,blank=True,on_delete=models.SET_NULL,related_name="item_requests")
    purpose=models.TextField()
    needed_by=models.DateField()
    delivery_location=models.CharField(max_length=180,blank=True)
    requested_approver=models.ForeignKey(Profile,null=True,blank=True,on_delete=models.SET_NULL,related_name="item_requests_to_approve")
    approval_task=models.OneToOneField(Task,null=True,blank=True,on_delete=models.SET_NULL,related_name="item_request_approval")
    status=models.CharField(max_length=20,choices=STATUSES,default="submitted")
    review_notes=models.TextField(blank=True)
    approved_by=models.ForeignKey(User,null=True,blank=True,on_delete=models.SET_NULL,related_name="approved_item_requests")
    approved_at=models.DateTimeField(null=True,blank=True)

    class Meta:
        ordering=["-created_at"]
        constraints=[models.UniqueConstraint(fields=["organization","number"],name="unique_item_request_number_per_org")]

    def clean(self):
        if self.department_id and self.department.organization_id!=self.organization_id: raise ValidationError("Department belongs to another organization.")
        if self.requested_approver_id and self.requested_approver.organization_id!=self.organization_id: raise ValidationError("Approver belongs to another organization.")

    @property
    def total_estimated_cost(self): return sum((line.estimated_cost for line in self.lines.all()),0)

    def __str__(self): return f"{self.number} · {self.requested_by.get_full_name() or self.requested_by.username}"
...
class ItemRequestLine(TimeStamped):
    ...
    def clean(self):
        if self.estimated_cost is not None and self.estimated_cost<=0: raise ValidationError("Estimated cost must be greater than zero.")

    def __str__(self): return f"{self.item} ({self.quantity})"
...
class ChatThread(TenantModel):
    ...
    created_by = models.ForeignKey(User,on_delete=models.PROTECT,related_name="created_chat_threads")
    participants = models.ManyToManyField(Profile,through="ChatMembership",related_name="chat_threads")
    is_active = models.BooleanField(default=True)

    class Meta:
        ordering = ["-updated_at"]
        constraints = [models.UniqueConstraint(fields=["organization","direct_key"],condition=~models.Q(direct_key=""),name="unique_direct_chat_per_org")]
...
class ChatMembership(TimeStamped):
    ...
    added_by = models.ForeignKey(User,null=True,blank=True,on_delete=models.SET_NULL,related_name="added_chat_memberships")
    is_admin = models.BooleanField(default=False)
    last_read_at = models.DateTimeField(default=timezone.now)

    class Meta:
        constraints = [models.UniqueConstraint(fields=["thread","profile"],name="unique_chat_membership")]

    def clean(self):
        if self.thread_id and self.profile_id and self.thread.organization_id != self.profile.organization_id:
...
class ChatInvitation(TimeStamped):
    ...
    thread = models.ForeignKey(ChatThread,on_delete=models.CASCADE,related_name="invitations")
    invitee = models.ForeignKey(Profile,on_delete=models.CASCADE,related_name="chat_invitations")
    invited_by = models.ForeignKey(User,on_delete=models.PROTECT,related_name="sent_chat_invitations")
    status = models.CharField(max_length=12,choices=STATUSES,default="pending")
    responded_at = models.DateTimeField(null=True,blank=True)

    class Meta:
        ordering = ["-created_at"]
        constraints = [models.UniqueConstraint(fields=["thread","invitee"],condition=models.Q(status="pending"),name="unique_pending_chat_invitation")]

    def clean(self):
        if self.thread_id and self.invitee_id and self.thread.organization_id != self.invitee.organization_id:
...
class ChatMessage(TimeStamped):
    ...
    sender = models.ForeignKey(Profile,on_delete=models.PROTECT,related_name="chat_messages")
    ...
    def clean(self):
        if self.thread_id and self.sender_id and self.thread.organization_id != self.sender.organization_id:
...
class ChatPresence(models.Model):
    profile = models.OneToOneField(Profile,on_delete=models.CASCADE,related_name="chat_presence")
